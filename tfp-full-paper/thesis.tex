...
In this paper, we describe a Haskell implementation of the reversible semantics proposed in~\cite{DBLP:conf/ppdp/MezzinaP17}. As such, our implementation defines a Haskell interpreter of message-passing programs written in the reversible process framework in~\cite{DBLP:conf/ppdp/MezzinaP17}. This allows us to assess in practice the benefits and features of the memories and mechanisms deployed in~\cite{DBLP:conf/ppdp/MezzinaP17} to enforce causally consistent reversibility. In developing our implementation, we have found the use of a functional programming language (Haskell in particular) a natural choice. Haskell has a strong history in language design. Its type system and mathematical nature allow us to faithfully implement the formal semantics and trust that our implementation is correct. In particular, algebraic data types (sums and products) are invaluable in expressing the grammars and recursive data structures underlying the model in~\cite{DBLP:conf/ppdp/MezzinaP17}.
...
\end{align*}
where 
$\myctxr{\ctx{T}_4}{\bullet}  =\ltout{\pS}{\mathsf{title}}{\bullet}$ is a the \emph{type context} (with hole $\bullet$) and, as before, $N_4$ represents participants not involved in the reduction.
...
to designing a better syntax for defining programs in Appendix~\ref{free-monad-dsl}.

\paragraph{Delegation via Abstraction Passing.}\label{abstraction-passing}

A delegation is when a participant can delegate (a part of)
their protocol to be fulfilled (i.e., implemented) by another participant. 
An example of
...
\noindent The final three constructors (\texttt{RecursionPoint}, \texttt{RecursionVariable}
, and \\ \texttt{WeakenRecursion}) are required for supporting nested
