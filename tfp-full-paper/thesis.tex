...
the running process associated to the monitor is currently involved in a backward step.
...
Recursive and terminated local types are denoted $\mu X. T$ and $\lend$, respectively. 
 ...
 Given these intuitions, the projection of a global type onto the various protocol participants is mostly straightforward, except for choice. 
 The formal definition is presented in Fig.~\ref{f:proj}.
...
We illustrate our  framework by presenting a reversible variant of the \emph{Three-Buyer protocol}   (see, e.g.,~\cite{CDYP2015})
with abstraction passing (code mobility), one of the 
distinctive traits of our framework. 
\erase{(In App.~\ref{app:choices} we discuss an additional example involving labeled choices.)}
...
She sends Bob's address (contained in the mobile code she received) to Vendor.
\item Vendor answers to Alice and Carol (who represents Bob) with the delivery date.
\end{enumerate}

%\newpage

We formalize this protocol is 
as the global type $G$ below.
%, another one involving Bob and Carol ($G_2$).
We write
$\gtcom{\p}{\{\q_1,\q_2\}}{U}{G}$
as a shorthand notation for 
$\gtcom{\p}{\q_1}{U}{\gtcom{\p}{\q_2}{U}{G}}$
(and similarly for local types).
We write $\thunkt$ to denote the type $\shot{\lend}$, associated to a \emph{thunk process} $\abs{x}{P}$ with $x \not \in \fn{P}$, written
$\thunkp{P}$. A thunk is an inactive process; it can be activated by applying to it a dummy name of type $\lend$, denoted $\dummyn$.
This way, we have 
$(\appl{\thunkp{P}}{\dummyn}) \fw P$.
\begin{align*}
...
\end{align*}
%where $T = \ltout{S}{\mathsf{address}}{\ltinp{S}{\mathsf{date}}{\lend}}$ is the protocol that Bob delegates to Carol.
where  $\mathsf{price}$ and $\mathsf{share}$ are base types treated as integers,
...
Observe how Bob's implementation sends part of its protocol to Carol in the form of a thunk containing 
his session name and address. This is how abstraction passing may serve to implement session delegation.
...
We use configuration $M_1$ to illustrate some forward and backward reductions.

From $M_1$ we could either undo the reduction (using Rule~\bkcolor{\textsc{(RInit)}})
...
\end{align*}
where $N_2$ stands for the running processes and monitors for Vendor, Bob, and Carol, not involved in the reduction.
We now have:
\begin{align*}
...
\end{align*}
where 
$\store_3  = \upd{x}{d},\upd{t}{\exBook}$ is
the resulting store,
...
Observe that the cursors in monitors $\ep{s}{\pS}$ and $\ep{s}{\pA}$ have evolved, and that message from $\pA$ to $\pS$ has now been moved to the input queue.

We illustrate  reversibility by showing how to return to $M_1$ starting from $M_3$.
We need to apply three rules: \bkcolor{\textsc{(RollS)}}, \bkcolor{\textsc{(RIn)}}, and \bkcolor{\textsc{(ROut)}}.
Reversibility is decoupled in the sense that there is no fixed order in which the latter two rules should be applied; below we give just a possible sequence.
...
\end{align*}
where
$\myctxr{\ctx{T}_4}{\bullet}  =\ltout{\pS}{\mathsf{title}}{\bullet}$ and
, as before, $N_4$ represents participants not involved in the reduction.
...
\end{align*}
Just as an application of Rule~\bkcolor{\textsc{(RollS)}} need not be immediately followed by 
an application of Rule~\bkcolor{\textsc{(RIn)}}, an application of Rule~\bkcolor{\textsc{(RIn)}} need not be immediately followed
by an application of Rule~\bkcolor{\textsc{(ROut)}}. A particular reduction from $M_5$ undoes the output at \pA:
\begin{align*}
M_5 & \bk  \news{s}(\,  
\np{\key{{\loc_1}}{\pS}}{ \conf{\inact}{\binp{\epS}{t}\bout{\epS}{price(t)}\bout{\epS}{price(t)} \\ 
