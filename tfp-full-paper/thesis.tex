...
\subsection{Implementing the process calculus in
...
As described in~\cite{DBLP:conf/ppdp/MezzinaP17}, we need to track information about two things: the type and the process.
...
\item
  ...
  Recall the process of the vendor in the ThreeBuyer example:
  \begin{align*}
  \text{Vendor} & =  \bout{d}{x:\tproj{G}{\pS}}\binp{x}{t}\bout{x}{price(t)}\bout{x}{price(t)} \binp{x}{ok}\binp{x}{a} \bout{x}{date}\inact
  \end{align*}

  Using the syntax introduced in Section \ref{free-monad-dsl} the vendor
  process can be implemented as:
...
\end{Shaded}
  ...
  The rest of the program depends on the assigned name. So for instance
  when we evaluate the \texttt{t\ \textless{}-\ H.receive} line
  (moving to state \(M_3\)), and then revert it, we must
  reconstruct a receive that assigns to \texttt{t}, because the
  following lines depend on the name \texttt{t}. \\

\item Function calls and their arguments \\
    
  Consider the transition from state \(M_7\) to \(M_8\)from Section \ref{the-process-model}. Once the thunk
  is evaluated, producing state \(M_8\), we lose all evidence that the
  ...
  not result in \(M_7\), so we need to keep track of function
  applications. The function and its argument are stored in a map
  indexed by a unique identifier \(k\). This identifier \(k\) is also
...
\item
  ...
  We consider again the implementation of the first 3 steps of the protocol:
...
\end{Shaded}
  ...
  Likewise when the starting from state \(M_3\) and undoing the receive,
  the value must be placed back into the queue. \\

  The implementation follows closely follows the formal semantics. 
