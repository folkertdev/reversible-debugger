...
Section~\ref{the-process-model} recalls the key notions from~\cite{DBLP:conf/ppdp/MezzinaP17}, useful to follow our Haskell implementation, which we detail 
...
There is a {configuration} for each participant: it includes a \emph{located process} that specifies asynchronous communication behavior and is subject to a \emph{monitor} that enables forward/backward steps at run-time. This monitor is obtained via the choreography, which describes the intended protocol among the {participants}. 
...
The third ingredient of the model in~\cite{DBLP:conf/ppdp/MezzinaP17}, not depicted in Fig.~\ref{f:model}, is the \emph{operational semantics} for configurations: it  is defined in terms of two reduction relations, forward and backward, denoted $\fw$  and $\bk$, respectively. We shall not recall these relations here; rather, we will introduce their key underlying intuitions by example.

\paragraph{Syntax of Configurations and Processes.}
The process language  $P, Q, \ldots$, given next, is a $\pi$-calculus with constructs for labeled (deterministic) choice, communication of abstractions, and function application:
...
and   enable the reversible semantics. 
...
\noindent
%tWe briefly discuss 
Value types $U$ include basic first-order values,   and  
...
Recursive and terminated local types are denoted $\mu X. T$ and $\lend$, respectively. 
 ...
 A novelty in the work of Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} are \emph{local types with history}  ($H, H'$). A type $H$ is a local type equipped with a
...
%(using two rules:~\fwcolor{\textsc{(Out)}} and ~\fwcolor{\textsc{(In)}}). 
This latter option would be as follows:
\begin{align*}
...
In a lazy language like Haskell, the tree may not be finite. If the tree is not finite,
...
requires enabling a language extension.
...
%that can be used from names, booleans and functions 
