...
In this paper, we describe a Haskell implementation of the reversible semantics proposed by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17}. As such, our implementation defines a Haskell interpreter of message-passing programs written in their reversible process framework. This allows us to assess in practice the benefits  of the mechanisms deployed by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} to enforce causally consistent reversibility. In developing our implementation, we have found the use of a functional programming language (Haskell in particular) a natural choice. Haskell has a strong history in language design. Its type system and mathematical nature allow us to faithfully capture the formal semantics given by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} and to trust that our implementation is correct and preserves causal consistency. In particular, algebraic data types (sums and products) are invaluable in expressing the grammars and recursive data structures underlying their model.

\paragraph{Organization.} %The rest of the paper is structured as follows.
Section~\ref{the-process-model} recalls the key notions from~\cite{DBLP:conf/ppdp/MezzinaP17}, useful to appreciate our Haskell implementation, which we detail 
...
The constructs in the second row appear only at run-time 
%(i.e., they do not appear in ``source'' programs) 
and are key to enable the reversible semantics. 
...
The \emph{tag} $\mytagg$ serves to determine whether 
...
As already mentioned, choreographies represent multiparty protocols expressed as global types $G, G', \ldots$. Global types can be \emph{projected} onto local types  ($T, T', \ldots$), one per participant. Their syntax follows~\cite{DBLP:conf/popl/HondaYC08}:
\begin{align*}
...
Recursive and terminated local types are denoted $\mu X. T$ and $\lend$, respectively. 
 ...
 A novelty in the work of Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} are \emph{local types with history}  $H, H', \ldots$. Intuitively, a type $H$ is a local type equipped with a
...
To address this, the choice made by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} (as in many other works) is to disallow different branches using different participants. In practice, 
this means that all branches must perform the same communication to satisfy the global type, even if the communication is not needed in every branch (some branches may have dummy communications). \modi{As we will see, in our Haskell implementation we use a different method where every
...
$$
%\end{document}
We now use $M$ to informally discuss forward and backward reduction rules for \fw and \bk; \modi{their formal definition is in~\cite{DBLP:conf/ppdp/MezzinaP17}}.
...
We need  three backward  rules: \bkcolor{\textsc{(RollS)}}, \bkcolor{\textsc{(RIn)}}, and \bkcolor{\textsc{(ROut)}}.
...
\end{align*}
where 
$\myctxr{\ctx{T}_4}{\bullet}  =\ltout{\pS}{\mathsf{title}}{\bullet}$ is a  \emph{type context} (with hole $\bullet$) and, as before, $N_4$ represents participants not involved in the reduction.
...
The Process calculus, Global and Local types and the information for reversal are all syntax trees. The first two are 
written by the programmer, and we that to be convenient. For that reason they are implemented as domain-specific languages (DSLs) in Haskell. 

\subsection{DSLs with the Free monad}

Free monads are a mechanism to conveniently construct domain-specific
languages. Because Haskell provides special syntax for working with
monads (do-notation), Free monads are a common technique.

The core idea is that we define a set of instructions. \texttt{Free}
constructs a tree out of these instructions, which can then be
interpreted and evaluated. The \texttt{Monad} instance for \texttt{Free}
makes it possible to use do-notation.
...
Now when we take the free monad of the \texttt{Operation} functor, this

generates a list (degenerate tree) of instructions. In a lazy language,
this list is not guaranteed to be finite. If the list is not finite,
evaluating it will cause a stack overflow. We use the \texttt{Void} type
to ensure the tree is finite with an \texttt{End} at the very bottom. An
alternative approach is to use existential quantification, but that
requires enabeling a language extension.

The \texttt{Void} type is the type with zero values. A value of the type
\texttt{Free\ Operation\ Void} by definition cannot end in the shape
\texttt{Pure\ \_}, because it requires a value of type \texttt{Void}
(which don't exist). It also cannot end in \texttt{Push\ \_\ \_} or
...
We define some wrappers around the constructors for convenience,
...
Finally, we expose a function to evaluate the structure we've built (but
only if it is finite). Typically, a \texttt{Free} monad is transformed
into some other monad, which in turn is evaluated. Here we interpred
first to \texttt{State}, and then evaluate that.
...
It turns out that we need some extra concepts to actually write programs with this syntax.
...
\noindent The final three constructors (\texttt{RecursionPoint}, \texttt{RecursionVariable}, and \\ \texttt{WeakenRecursion}) are required for supporting nested
...
As described by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17}, we need to track information about the local type and the process.
...
\item \emph{Used variable names in receives}.
  ...
  Using the syntax presented in Appendix~\ref{free-monad-dsl}, we can write this
  process as:
\begin{Shaded}
\begin{Highlighting}[]
...
\end{Shaded}
  ...
  Our implementation follows closely follows the formal semantics by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17}. 
...
With all the definitions in pace, we can now define the forward and backward
evaluation of our system. Our aim is to implement the reduction relations \fw and \bk with the types:
\begin{Shaded}
\begin{Highlighting}[]
...
The semantics by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} guides how we store the execution
state. Some data is bound to its location (e.g., the process that
is running) and other data is bound to its participant (e.g., the
local type).
...
As mentioned in the Introduction, causal consistency is a key correctness criterion for a reversible semantics. This property has been proved Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} for the formal model laid out in Section \ref{the-process-model}. One may then wonder, does our Haskell implementation preserve this property? 
...
Only in this state is the appropriate data, of the appropriate type, available.
...
we claim that the Haskell implementation is faithful enough to the formal semantics, and therefore that the causal consistency property is preserved.
...
locations left. For code please see Appendix \ref{scheduling-code}.
...
testing the reversible semantics. \modi{The type system can actually guarantee
...
We have given a functional implementation of the reversible semantics by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} using the Haskell
programming language. By embedding the reversible semantics we can now run and
verify our example programs automatically and inspect them
interactively.

We have seen that the formal programming model by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} paper can be split into three core components: (i)~a process calculus, (ii)~multiparty
