...
The Process calculus, Global and Local types, and the information for reversal are all represented as syntax trees. The first two are 
written by the programmer, and we want to provide a convenient way to specify them as domain-specific languages (DSLs) in Haskell. 
...
We define a data type with our instructions, and make sure it has a \texttt{Functor} instance (meaning that there exists a function \texttt{fmap :: (a -> b) -> Operation a -> Operation b}).
Given an instance of \texttt{Functor}, \texttt{Free} will give us the free monad on that functor. 
In effect this describes a tree (in this particular case a list) of instructions. 

In a lazy language like Haskell, the tree is not guaranteed to be finite. If the tree is not finite,
...
\noindent We have to use \texttt{tipe} because \texttt{type} is a keyword in Haskell. 
The final three constructors (\texttt{RecursionPoint}, \texttt{RecursionVariable}, and \\ \texttt{WeakenRecursion}) are required for supporting nested
...
\OtherTok{messages ::}\NormalTok{ participant }\OtherTok{->} \DataTypeTok{List}\NormalTok{ participant }
...
\OtherTok{oneOf ::}\NormalTok{ participant }\OtherTok{->}\NormalTok{ participant }
      \OtherTok{->} \DataTypeTok{List}\NormalTok{ (}\DataTypeTok{String}\NormalTok{, }\DataTypeTok{GlobalType}\NormalTok{ participant u a) }
...
\NormalTok{recurse cont }\FunctionTok{=} \DataTypeTok{Free}\NormalTok{ (}\DataTypeTok{R}\NormalTok{ cont)}

\OtherTok{weakenRecursion ::} \DataTypeTok{GlobalType}\NormalTok{ p u a }\OtherTok{->} \DataTypeTok{GlobalType}\NormalTok{ p u a}
\NormalTok{weakenRecursion cont }\FunctionTok{=} \DataTypeTok{Free}\NormalTok{ (}\DataTypeTok{Wk}\NormalTok{ cont)}

\OtherTok{recursionVariable ::} \DataTypeTok{GlobalType}\NormalTok{ p u a }
\NormalTok{recursionVariable }\FunctionTok{=} \DataTypeTok{Free} \DataTypeTok{V}
