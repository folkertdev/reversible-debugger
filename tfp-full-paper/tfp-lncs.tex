...
\checkthis{
The fundamental problem is that a global type is written sequentially. It may represent implicit paralellism but the scheduler 
currently just runs the global type that it is given in the specified order.
...
The definitions correspond almost directly \checkthis{(The \texttt{WeakenRecursion} constructor is added to support nested recursion, which the formal model does not support)}. Moreover, we know that these are
