...
in  \secref{implementing-the-ppdp17-calculus-in-haskell}.
 ...
 The implementation is available at \url{https://github.com/folkertdev/reversible-debugger}.
...
\emph{choreography}, which represent the communicating partners (\emph{participants}) and a description of their intended governing protocol, respectively. 
There is a {configuration} for each participant: it includes a \emph{located process} that relies on asynchronous communication and is subject to a \emph{monitor} that enables forward/backward steps at run-time and is obtained from the choreography.
...
\end{align*}
In $\bsel{u}{\lbl_i. P_i}_{i\in I}$ and $\bbra{u}{\lbl_i:P_i}_{i \in I}$, we use $I$ to denote some finite index set. 
...
\text{Bob} & =  \binp{d}{z{\,:\,}\tproj{G}{\pB}}\binp{z}{p}\binp{z}{h}\bout{z}{ok}\bout{z}{ok}\bout{z}{h}
 \bbout{z}{\thunkp{\bout{z}{\text{`9747'}}\binp{z}{d}\inact}}\inact
  \\
...
$$
%\end{document}
We now use configuration $M$ to discuss the reduction relations \fw and \bk; below we shall refer to forward and backward reduction rules {defined in Mezzina and P\'{e}rez's paper~\cite[\S\,2.2.2]{DBLP:conf/ppdp/MezzinaP17}}.
...
One particular backward reduction is the one that
...
\begin{align*}
M_7 = ~ &   \news{s}(\,  \np{\key{{\loc_3}}{\pB}}{ \conf{\inact}{\bbout{\epB}{\thunkp{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}}\inact}} 
\\
...
\end{align*}
where 
$\thunkp{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}$
...
\\
& 
\Par N_5 \Par \codah{s}{h_7 \cons \valueq{\pB}{\pC}{\thunkp{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}}}{\emp}{}~) 
\end{align*}
where
$\store_9 = \store_8 \upd{code}{\thunkp{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}}$.
We now may 
...
%\\
%& \Par 
\np{\key{{\loc_4}}{\pC}}{\conf{\inact}{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}} 
\!\!  \Par N_6 
\\
& \!\!\Par \hmoni{\ep{s}{\pB}}{\myctxr{\ctx{T}_7}{\ltout{C}{\thunkt}{\past\ltout{\pS}{\mathsf{address}}{\ltinp{\pS}{\mathsf{date}}{\lend}}}}}{z,p,h}{\store_7} 
\\
...
\\
& 
\!\!\Par \codah{s}{h_7 \cons \valueq{\pB}{\pC}{\thunkp{\bout{\epB}{\text{`9747'}}\binp{\epB}{d}\inact}}}{\emp}{}~) 
= M_9
\end{align*}
...
Local types are obtained by from the global type via projection, which we implement following \secref{projection}, 
...
\checkthis{As explained in \secref{ss:processes}, each protocol participant has its own monitor with its own store. 
Because these stores are not shared, all variables occurring in the arguments to operators and in function bodies 
...
\NormalTok{        (}\DataTypeTok{StateT}\NormalTok{ (}\DataTypeTok{Participant}\NormalTok{, }\DataTypeTok{Int}\NormalTok{) }
\NormalTok{        (}\DataTypeTok{Free}\NormalTok{ (}\DataTypeTok{ProgramF} \DataTypeTok{Value}\NormalTok{)) a)}
        \KeywordTok{deriving} 
...
Following Fig.~\ref{f:model}, \checkthis{our implementation uses a global type specification to obtain a local type (of type \texttt{LocalType}), one per participant, by means of projection. This is  implemented as described in  \secref{projection}}.
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{message ::}\NormalTok{ participant }\OtherTok{->}\NormalTok{ participant }\OtherTok{->}\NormalTok{ tipe }
\OtherTok{        ->} \DataTypeTok{GlobalType}\NormalTok{ participant tipe () }
...
\checkthis{
In general, a key issue is that a global type is written sequentially, while it may represent implicit parallelism, as explained in \secref{ss:types}. Currently, our implementation just executes the global type with the order given by the programmer. 
It should be possible to execute communication actions in different but equivalent orders; these optimizations are beyond the scope of our current implementation. 
}
...
\KeywordTok{data} \DataTypeTok{GlobalTypeF}\NormalTok{ u next }
    \FunctionTok{=} \DataTypeTok{Transaction}\NormalTok{ \{}\FunctionTok{..}\NormalTok{\} }\FunctionTok{|} \DataTypeTok{Choice}\NormalTok{ \{}\FunctionTok{..}\NormalTok{\}  }\FunctionTok{|} \DataTypeTok{RecursionPoint}\NormalTok{ next }
    \FunctionTok{|} \DataTypeTok{RecursionVariable} \FunctionTok{|} \DataTypeTok{End} 
    \FunctionTok{|} \DataTypeTok{WeakenRecursion}\NormalTok{ next}
\end{Highlighting}
\end{Shaded}
...
The definitions correspond almost directly: \checkthis{the \texttt{WeakenRecursion} constructor is added to support nested recursion, which the formal model does not explicitly represent}. Moreover, we know that these are
...
We presented a functional implementation of the (reversible) MP model~\cite{DBLP:conf/ppdp/MezzinaP17} using   Haskell. By embedding this reversible semantics we can now execute our example programs automatically and inspect them
interactively.
...
and to the TFP'18 co-chairs (Micha\l{} Pa\l{}ka and Magnus Myreen)
for their useful remarks and suggestions, which led to substantial improvements.
P\'{e}rez is also affiliated to 
CWI, Amsterdam, The Netherlands and to
the NOVA Laboratory for Computer Science and Informatics (supported by FCT grant NOVA LINCS PEst/UID/CEC/04516/2013), Universidade Nova de Lisboa, Portugal.

This research has been partially supported by the Undergraduate School of Science and the Bernoulli Institute of the University of Groningen. We also acknowledge support from the 
