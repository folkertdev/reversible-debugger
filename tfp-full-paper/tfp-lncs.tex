...
Composition allows filling the holes with (possibly incomplete) subprograms. 
...
The \texttt{Value} type has some extra constructors in comparison to the formal description to create more interesting examples.
We have added integers, strings, and basic integer and comparison operators. The label used to differentiate the different cases of offer and select statements
is also part of the value type. Reference refers to the variable case of the formal definition.
...
Because variables are not shared between sessions, all variables (in the arguments to operators and in function bodies) 
...
The projection of a global type onto its participants is implemented as described in section \ref{projection}.
Much like the process syntax, the specification of global types closely mimics the formal definition:
...
\end{Shaded}
  ...
  Our implementation closely follows the formal semantics of the MP model. 
  ...
  history stack.  {When the receive is reversed}, the queue pops the value
