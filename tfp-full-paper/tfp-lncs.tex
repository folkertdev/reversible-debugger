...
\end{itemize}
These intuitions are formalized by the operational semantics of the MP model; see~\cite{DBLP:conf/ppdp/MezzinaP17} for details.
...
\checkthis{Our implementation relies on broadcasts to communicate choices to all protocol participants; this reduces the need for explicit communications in global types}. 
...
$$
%\end{document}
We now use $M$ to informally discuss reduction rules for \fw and \bk; {their formal definition is in Mezzina and P\'{e}rez's paper~\cite{DBLP:conf/ppdp/MezzinaP17}}.
