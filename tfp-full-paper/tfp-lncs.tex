...
%(Appendix \ref{fix}) 
to factor out recursion from the grammar.
...
\NormalTok{complex }\FunctionTok{=} \DataTypeTok{Fix}\NormalTok{ (}\DataTypeTok{Add}\NormalTok{ (}\DataTypeTok{Fix}\NormalTok{ (}\DataTypeTok{Literal} \DecValTok{40}\NormalTok{)) (}\DataTypeTok{Fix}\NormalTok{ (}\DataTypeTok{Literal} \DecValTok{2}\NormalTok{)))}
\end{Highlighting}
\end{Shaded}
We will describe a better way of writing \texttt{Fix}ed values in Appendix~\ref{free-monad-dsl}.
\end{example}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{example }\FunctionTok{=} \DataTypeTok{Fix} 
...
\noindent
While the above is easily manipulable with pattern matching and
...
and in general there is too much syntactical clutter. We will describe how 
to design a better syntax for writing programs in Appendix~\ref{free-monad-dsl}.
