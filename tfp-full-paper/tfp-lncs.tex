...
In this paper we describe a Haskell implementation of the reversible semantics  by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17}. As such, our implementation defines a Haskell interpreter of message-passing programs written in their reversible process framework. This allows us to assess in practice the mechanisms deployed by Mezzina and P\'{e}rez to enforce causally consistent reversibility. The use of a functional programming language (Haskell in particular) is a natural choice for developing our implementation. Haskell has a strong history in language design. Its type system and mathematical nature allow us to faithfully capture the formal reversible semantics and to trust that our implementation  correctly preserves causal consistency. In particular, algebraic data types (sums and products) are invaluable to express  the grammars and recursive data structures underlying their model.
...
The projection of a global type $G$ onto a protocol participant $\p$, denoted $\tproj{G}{\gpart{p}}$,  is defined in Fig.~\ref{f:proj}. 
 ...
 This is a condition adopted by Mezzina and P\'{e}rez~\cite{DBLP:conf/ppdp/MezzinaP17} but also by several other works, as it allows to ensure implementability of  multiparty session types.
...
$$
%\end{document}
We now use $M$ to informally discuss freduction rules for \fw and \bk; {their formal definition is in~\cite{DBLP:conf/ppdp/MezzinaP17}}.
...
From $M_1$ we could either undo this reduction %(using Rule~\bkcolor{\textsc{(RInit)}})
