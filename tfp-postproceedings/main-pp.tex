...
%
\title{Reversible Choreographies in Haskell 
%\thanks{This work was partially supported by the EU COST action IC1405 (Reversible Computation -- Extending Horizons of Computing).}
}
%
...
\keywords{Reversibility, Message-passing concurrency, Haskell.}
\end{abstract}
%
%
%
...
\modif{intuitively denotes the (local) contribution of   $\gpart{r}$ to the overall choreographic behavior that $G$  specifies.}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data} \DataTypeTok{MyParticipants} \FunctionTok{=} \DataTypeTok{A} \FunctionTok{|} \DataTypeTok{B} \FunctionTok{|} \DataTypeTok{C} \FunctionTok{|} \DataTypeTok{V} \KeywordTok{deriving}\NormalTok{ (}\DataTypeTok{Show}\NormalTok{, }\DataTypeTok{Eq}\NormalTok{, }\DataTypeTok{Ord}\NormalTok{)}
...
\NormalTok{    GlobalType.transactions }\DataTypeTok{V}\NormalTok{ [}\DataTypeTok{A}\NormalTok{, }\DataTypeTok{B}\NormalTok{] }\DataTypeTok{Price} 
\NormalTok{    GlobalType.transaction }\DataTypeTok{A} \DataTypeTok{B} \DataTypeTok{Share} 
\NormalTok{    GlobalType.transactions }\DataTypeTok{B}\NormalTok{ [}\DataTypeTok{A}\NormalTok{, }\DataTypeTok{V}\NormalTok{] }\DataTypeTok{Ok} 
...
\OtherTok{derivedTypeForA ::} \DataTypeTok{LocalType} \DataTypeTok{MyType}
\NormalTok{derivedTypeForA }\FunctionTok{=} \KeywordTok{do}
\NormalTok{    send }\DataTypeTok{V} \DataTypeTok{Title}
\NormalTok{    receive }\DataTypeTok{V} \DataTypeTok{Price}
\NormalTok{    send }\DataTypeTok{B} \DataTypeTok{Share}
\NormalTok{    receive }\DataTypeTok{B} \DataTypeTok{Ok}
\end{Highlighting}
\end{Shaded}

\subsection{A Language}\label{a-language}
The process language to be used with our types needs at least instructions
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{uniqueVariableName ::} \DataTypeTok{HighLevelProgram} \DataTypeTok{Identifier}
\NormalTok{uniqueVariableName }\FunctionTok{=} \KeywordTok{do}
\NormalTok{    (location, participant, n) }\OtherTok{<-}\NormalTok{ State.get}
\NormalTok{    State.put (location, participant, n }\FunctionTok{+} \DecValTok{1}\NormalTok{)}
...
\NormalTok{    (_, participant, _) }\OtherTok{<-}\NormalTok{ State.get}
    \DataTypeTok{HighLevelProgram} \FunctionTok{$}\NormalTok{ lift }\FunctionTok{$} 
\NormalTok{        liftFree (}\DataTypeTok{Send}\NormalTok{ participant value ())  }
...
\NormalTok{    variableName }\OtherTok{<-}\NormalTok{ uniqueVariableName }
    \DataTypeTok{HighLevelProgram} \FunctionTok{$}\NormalTok{ lift }\FunctionTok{$} 
\NormalTok{        liftFree (}\DataTypeTok{Receive}\NormalTok{ participant variableName ())}
...
\NormalTok{terminate }\FunctionTok{=} \DataTypeTok{HighLevelProgram}\NormalTok{ (lift }\FunctionTok{$} \DataTypeTok{Free} \DataTypeTok{NoOp}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

We can now write a correct implementation of \texttt{A}'s local type.

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{aType ::} \DataTypeTok{LocalType} \DataTypeTok{MyType}
\NormalTok{aType }\FunctionTok{=} \KeywordTok{do}
\NormalTok{    send }\DataTypeTok{V} \DataTypeTok{Title}
\NormalTok{    receive }\DataTypeTok{V} \DataTypeTok{Price}
\NormalTok{    send }\DataTypeTok{B} \DataTypeTok{Share}
\NormalTok{    receive }\DataTypeTok{B} \DataTypeTok{Ok}

\NormalTok{alice }\FunctionTok{=}\NormalTok{ H.compile }\StringTok{"Location1"} \StringTok{"A"} \FunctionTok{$} \KeywordTok{do} 
    \KeywordTok{let}\NormalTok{ share }\FunctionTok{=} \DataTypeTok{VInt} \DecValTok{42} 
\NormalTok{    H.send (}\DataTypeTok{VString} \StringTok{"address"}\NormalTok{ )}
\NormalTok{    price }\OtherTok{<-}\NormalTok{ H.receive }
\NormalTok{    H.send share}
\NormalTok{    ok }\OtherTok{<-}\NormalTok{ H.receive }
\NormalTok{    H.terminate}
\end{Highlighting}
\end{Shaded}

\texttt{HighLevelProgram} is transformed into a \texttt{Program} by
evaluating the \texttt{StateT}; this puts the correct owner and unique
variable names into the tree, and then transforming \texttt{Free} to
\texttt{Fix} by putting \texttt{NoOp} at the leaves where needed.

\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{freeToFix ::} \DataTypeTok{Free}\NormalTok{ (}\DataTypeTok{ProgramF}\NormalTok{ value) a }\OtherTok{->} \DataTypeTok{Program}\NormalTok{ value}
\NormalTok{freeToFix (}\DataTypeTok{Pure}\NormalTok{ n) }\FunctionTok{=} \DataTypeTok{Fix} \DataTypeTok{NoOp} 
\NormalTok{freeToFix (}\DataTypeTok{Free}\NormalTok{ x) }\FunctionTok{=} \DataTypeTok{Fix}\NormalTok{ (fmap freeToFix x)}

\OtherTok{compile ::} \DataTypeTok{Location} 
        \OtherTok{->} \DataTypeTok{Participant} 
        \OtherTok{->} \DataTypeTok{HighLevelProgram}\NormalTok{ a }
        \OtherTok{->} \DataTypeTok{Program}\NormalTok{ value}
\NormalTok{compile location participant (}\DataTypeTok{HighLevelProgram}\NormalTok{ program) }\FunctionTok{=} 
\NormalTok{    freeToFix }\FunctionTok{$}\NormalTok{ runStateT program (location, participant, }\DecValTok{0}\NormalTok{) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{bob }\FunctionTok{=}\NormalTok{ H.compile }\StringTok{"Location1"} \StringTok{"B"} \FunctionTok{$} \KeywordTok{do} 
\NormalTok{    thunk }\OtherTok{<-} 
\NormalTok{        H.function }\FunctionTok{$}\NormalTok{ \textbackslash{}_ }\OtherTok{->} \KeywordTok{do}
\NormalTok{            H.send (}\DataTypeTok{VString} \StringTok{"Lucca, 55100"}\NormalTok{)}
\NormalTok{            d }\OtherTok{<-}\NormalTok{ H.receive}
\NormalTok{            H.terminate}

\NormalTok{    price }\OtherTok{<-}\NormalTok{ H.receive }
\NormalTok{    share }\OtherTok{<-}\NormalTok{ H.receive }
    \KeywordTok{let}\NormalTok{ verdict }\FunctionTok{=}\NormalTok{ price }\OtherTok{`H.lessThan`} \DataTypeTok{VInt} \DecValTok{79}
\NormalTok{    H.send verdict }
\NormalTok{    H.send verdict }
\NormalTok{    H.send share}
\NormalTok{    H.send thunk }


\NormalTok{carol }\FunctionTok{=}\NormalTok{ H.compile }\StringTok{"Location1"} \StringTok{"C"} \FunctionTok{$} \KeywordTok{do} 
